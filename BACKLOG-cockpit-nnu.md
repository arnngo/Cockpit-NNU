...
- **Resumo vira página de decisão (UX executivo)** — a aba Resumo foi reorganizada para responder "qual o status, o que trava, qual a próxima ação" em 10 segundos: (1) **faixa Status executivo** no topo, colorida pela saúde, com a **trava principal** em destaque e régua de Gate atual · IPPE · Avanço total · IPPL · Risco · Prioridade · Atualizado; (2) **Próxima ação** (a aberta de maior prioridade) destacada à direita da faixa — borda vermelha se crítica — sem repetir na lista abaixo; (3) **Resumo executivo** (texto do gestor, reaproveita `statusNota`); (4) **Caminho crítico** unificado e ranqueado por urgência, com etiqueta de natureza (trava do gate / risco / externa / interna); (5) **Riscos no resumo** — risco executivo + contagem + top 3; se a matriz estiver vazia, vira **pendência de governança** ("matriz não preenchida"); (6) **IPPL com composição** (o que falta pro lançamento); (7) **Evolução como tendência** (variação semanal; "histórico insuficiente" com 1 medição). Lista de Ações da semana enxuta (só o título). Rodapé encurtado.
