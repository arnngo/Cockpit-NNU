...
> **Pacote v4** · TEMPLATE 2026.07.5 · `dados.json` com a **base real atualizada** (13 frentes). Aba **Resumo** redesenhada como página de decisão: faixa de status executivo com trava principal e **próxima ação** em destaque, caminho crítico ranqueado, **riscos no resumo** (com pendência de governança se a matriz estiver vazia), IPPL com composição e evolução como tendência. Três senhas — leitura `123`, edição `URB` (registra quem atualiza), master `opus890` (gerencia editores/departamentos, edita/remove comentários). Itens com Nº do processo no topo; datas abrindo em 2026.
